...
class Path(Element):
    ...
    def get_path(cls, x: float, y: float, width: float, height: float) -> list[str]:
        ...
        return " ".join(
            [
                f"M{round_coordinate(x)} {round_coordinate(y)}",
                f"h{round_coordinate(width)}",
                f"v{round_coordinate(height)}",
                f"h{-1 * round_coordinate(width)}",
                f"v{-1 * round_coordinate(height)}Z",
            ]
        )
