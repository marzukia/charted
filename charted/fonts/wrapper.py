...
class Font:
    ...
    def measure_height(self, size: int | None = None) -> float:
        ...
        definitions = self.definitions.get(str(size if size is not None else self.size), {})
        if not definitions:
        ...
        return max(heights) if heights else float(size if size is not None else self.size)
